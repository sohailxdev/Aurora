...
export default function EnhancedCartItem({ product, onQuantityChange, onRemove }: EnhancedCartItemProps) {
    ...
    <div className="flex border gap-4 p-4 rounded-md">
        ...
        <div className="flex items-center gap-4">
          <div className="flex items-center gap-2">
            <Button
              ...
              className="h-8 w-8"
              onClick={() =>
                onQuantityChange(product.id, product.sku, product.quantity, product.quantity - 1, product.sku_quantity)
              }
      ...
      <div className="text-right flex flex-col justify-between">
        <div className="text-right">
          {hasOffer ? (
            <>
              <p className="font-medium text-gray-500 line-through">₹{Math.round(offerResult.originalPrice)}</p>
              <p className="font-medium">₹{Math.round(offerResult.discountedPrice)}</p>
            ...
            <>
              <p className="font-medium text-gray-500 line-through">₹{Math.round(product.price * product.quantity)}</p>
              {product.discount.valueType === "PERCENTAGE" ? (
                ...
                <>
                  <p className="font-medium">
                    ₹{Math.round(product.price * product.quantity - product.discount.discountValue * product.quantity)}
                  </p>
                  <small className="text-green-600">₹{product.discount.discountValue} off</small>
                </>
              )}
        ...
        <Button variant="ghost" size="icon" onClick={() => onRemove(product.sku)}>
